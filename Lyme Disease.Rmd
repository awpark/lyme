...
* consider tweaking incidence to be cases per 100,000 ppl

```{r}

x <- ld.tick.county %>% filter(Presence_current==1) %>% group_by(State,year) %>% summarize(tot.cases=sum(cases))
y <- inner_join(x,av.jobs,by=c("State","year"))
y250up <- y %>% filter(tot.cases>=250)
ggplot(y,aes(x=av.UR,y=tot.cases,col=as.factor(year)))+geom_point()

z <- pop %>% group_by(state_name,year) %>% summarize(tot.pop=sum(pop))
z %<>% rename(State=state_name)

z <- inner_join(z,y)
z %<>% mutate(incidence=tot.cases/tot.pop)

z_big <- z %>% filter(incidence>=0.0003)
