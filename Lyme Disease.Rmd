...
risk<-inner_join(ld.pres.total,jobs)
risk_count<-risk%>%filter(DeerDensityMax>1)%>%filter(DeerDensityMax<4)%>%filter(UnemploymentRate<5)
...
```{r}
## Do cases occur only where ticks are present?
pres_plot<-ld.tick %>% filter(cases>0) %>% ggplot(aes(x=as.factor(Presence_current),y=cases))+geom_boxplot(fill="#6699FF")+scale_y_log10()+labs(x="Current Tick Presence",y="Cases",title="Do cases occur only where ticks are present?",caption="0=No ticks present || 1=Ticks present")
...
map_2<-ggplot(heat,aes(x=long,y=lat,group=group))+geom_polygon(col=ifelse(heat$SC %in% cnames.1$SC,"red","gray40"),fill=ifelse(heat$av.cases>1,"gold","powderblue"))+scale_fill_continuous()+coord_map("polyconic")+geom_path(lwd=0.1)+labs(x="Longitude",y="Latitude",title="Counties with Cases>1 where Ticks aren't Present")
...
## Total Lyme Disease case distribution map: This gives us a nice visual, but not one that takes population into account for increased case numbers
map_3<-ggplot(data=heat,aes(long,lat,group=group))+geom_polygon(data=heat,aes(fill=(tot.cases+1)))+scale_fill_gradientn(colours=rev(rainbow(7)),trans="log10")+labs(x="Longitude",y="Latitude",colours="Total Cases",title="County-based Lyme Disease Case Counts",fill="Total Cases")+coord_map("polyconic")

##Lyme Disease incidence distribution map: Using incidence allows us to control for influxes in total case count due to differences in population. The number of cases per 100,000 people, if every county had 100,000 people this is how many of them would have contracted Lyme Disease. This gives us a better visualization of exactly where Lyme Disease hot spots occur

map_4<-ggplot(data=heat,aes(long,lat,group=group))+geom_polygon(data=heat,aes(fill=(incidence+1)))+scale_fill_gradientn(colours=rev(rainbow(7)),trans="log10")+labs(x="Longitude",y="Latitude",colours="Incidence",title="County-based Lyme Disease Incidence",fill="Incidence",caption="Incidence=100,000*(total cases/total population)")+coord_map("polyconic")

## So what factors can influence Lyme Disease distribution and does data support previous research on Lyme disease general patterns?: Anova shows that the model containing all predictors had the best fit with the data (lowest AIC)

tbl<-(anova(gm_1,gm_2,gm_3,gm_4))
...
tbl$`Chi Df`<-NULL
tbl$`Pr(>Chisq)`<-NULL
tbl%<>%mutate(ΔAIC=(AIC-AIC[4]))
rownames(tbl)[1]<-"-UR & -DD"
rownames(tbl)[2]<-"-UR"
rownames(tbl)[3]<-"-DD"
rownames(tbl)[4]<-"All"
tbl<-tbl[,c(1,2,4,3)]
...
temp_plot<-ggplot(x_big,aes(av.temp,incidence))+geom_point(aes(col=as.factor(year)))+geom_smooth(method="lm",formula=y ~ poly(x, 2))+labs(x="Average Temperature",y="Incidence",title="State Incidence vs. Average Temperature")+guides(color=guide_legend(title="Year"))


#Precipitation:Cases and incidence increase with increased precipitation
prcp_plot<-ggplot(x_big,aes(av.prcp,incidence))+geom_point(aes(col=as.factor(year)))+geom_smooth(method="lm")+labs(x="Average Precipitation",y="Incidence",title="State Incidence vs. Average Precipitation")+guides(color=guide_legend(title="Year"))
...
All_UR<-ggplot(u,aes(x=year,y=av.UR,group=State,colour=State))+geom_line()+labs(x="Year",y="Average Unemployment Rate (%)",title="State Annual Unemployment Rates")

#How does incidence relate to average unemployment rate?: As unemployment rate increases,incidence decreases

tmp8<-inner_join(ld.pop.edit,ld.jobs.total)
tmp8%<>%filter(cases>0)%<>%filter(incidence>=230)
UR_plot<-ggplot(tmp8,aes(x=UnemploymentRate,y=incidence))+geom_point(aes(col=as.factor(year)))+geom_smooth(method="lm")+labs(x="Unemployment Rate",y="Incidence",title="County-Based Unemployment vs Incidence")+guides(color=guide_legend(title="Year"))

#How does deer density effect incidence and total cases?

ld.pop.edit%<>% mutate(incidence=(100000*(cases/pop)))
tmp4<-ld.pop.edit%>%filter(cases>0)

ggplot(tmp4,aes(x=(DeerDensityMax),y=incidence,group=DeerDensityMax))+geom_boxplot(fill="#6699FF")+labs(x="Deer Density",y="Incidence",title="County-scaled Deer Density vs Incidence",caption="*0 being lowest deer density, 4 being highest")+scale_y_log10()

#At risk counties with intermediate deer density, low unemployment rate

tmp7<-inner_join(risk_count,cnames.1,by="SC")
boom<-unique(tmp7$SC)

points<-data.frame(c(long=-88.86289,-85.07850,-86.26166,-87.57946,-91.15185,-95.29720,-94.84457,-97.48877,-85.68584,-85.90643,-84.37090,-85.52541,-85.54546,-86.00669,-96.16810,-97.22521,-97.38850,-82.46868,-79.92475,-79.44920,-79.12547,-79.43200,-97.80676,-99.88660,-79.41768),c(lat=40.51385,41.09826,41.32744,37.99856,40.89773,38.90956,38.90097,37.67484,38.18764,42.59368,42.60514,42.24704,43.03200,42.98903,41.28161,46.93097,47.93365,40.10705,40.92065,41.20140,40.64563,40.35915,32.77319,32.29763,37.80662))
colnames(points)[1]<-"long"
colnames(points)[2]<-"lat"

ggplot(data=heat,aes(long,lat,group=group))+geom_polygon(data=heat,aes(fill=(incidence+1)))+scale_fill_gradientn(colours=rev(rainbow(7)),trans="log10")+labs(x="Longitude",y="Latitude",colours="Incidence",title="At Risk Counties",fill="Incidence")+coord_map("polyconic")+ geom_point(data=points,aes(x=long,y=lat,group=long),size=1)
