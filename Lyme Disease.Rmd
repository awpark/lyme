...
* where do the 0 tick Lyme cases occur geographically
* when do the same cases occur


##How can we map cases

```{r}

library(maps)
county_map<-map_data("county")
ggplot(county_map,aes(x=long,y=lat,group=group))+geom_polygon(col="blue",fill=ifelse(county_map$group>1500,"red","green"))#+geom_path(lwd=0.1,col="red")
...
ld.av <- aggregate(ld.tick$cases,by=list(ld.tick$SC),FUN=mean)
names(ld.av)<-c("SC","av.cases")
tmp<-inner_join(county_map,ld.av)

ggplot(tmp,aes(x=long,y=lat,group=group,fill=log10(av.cases+1)))+geom_polygon()+scale_fill_continuous()+coord_map("polyconic")
