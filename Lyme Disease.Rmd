...
ggplot(pres.1,aes(x=(cases),y=log10(prcp)))+geom_point(data=subset(pres.1,year==2000,stat=identity))

ggplot(pres.1,aes(x=(cases),y=log10(prcp)))+geom_point(data=subset(pres.1,year==2001,stat=identity))

ggplot(pres.1,aes(x=(cases),y=log10(prcp)))+geom_point(data=subset(pres.1,year==2002,stat=identity))
...
```{r}
#Building dataframe of average unemployment rate and average cases per state 2007-2015
...
ggplot(av.jobs,aes(x=av.cases,y=av))
...
```{r}
library(reshape2)
melted<-melt(av.jobs,id.vars="State",value.name = "av.UR",variable.name = "year")

ggplot(melted,aes(x=year,y=av.UR,group=State,colour=State))+geom_line()
```
